...
@bp.route('/games')
def list_games():
    games = Game.query.order_by(Game.week, Game.start_time).all()
...
def my_results():
    """
    Optional: ?week=<int>  → only that week. Otherwise, season-to-date for this user.
    """
    week = request.args.get('week', type=int)

    # Load games (filter week if provided)
    gq = Game.query
    if week is not None:
        gq = gq.filter_by(week=week)
    games = gq.all()
    games_by_id = {g.id: g for g in games}

    # Load this user's picks for those games
    pq = db.session.query(Pick).join(Game, Pick.game_id == Game.id).filter(Pick.user_id == current_user.id)
    if week is not None:
        pq = pq.filter(Game.week == week)
    picks = pq.all()

    # Build rows with outcome + points
    rows = []
    total_points = 0.0
    ...
    for p in picks:
        ...
        if not g:
            continue
        res = game_result_against_spread(g)   # "home" | "away" | "push" | None
        pts = points_for_pick(p, g)           # 1.0 | 0.5 | 0.0 | None
        ...
        if res is None:
            ...
            if res == "push":
                ...
                side = "home" if p.chosen_team == g.home_team else "away"
                if side == res:
                    outcome = "Win"
                    wins += 1
                else:
        ...
        rows.append({
            ...
            "final": (None if g.final_score_home is None else f"{g.final_score_away}-{g.final_score_home}"),
            "outcome": outcome,
            "points": None if pts is None else round(pts, 1),
            "start_time": g.start_time,
        })

    # Sort by (week, start_time)
    rows.sort(key=lambda r: (r["week"], r["start_time"]))
    ...
    return render_template("my_results.html", rows=rows, week=week, summary=summary)

@bp.route('/admin/scores', methods=['GET', 'POST'])
@login_required
def admin_scores():
    admin_required()  # ensure only admins

    # choose week
    week = request.values.get('week', type=int)  # works for GET/POST
    if week is None:
        # default to the smallest upcoming, or 1
        g = Game.query.order_by(Game.week.asc()).first()
        week = g.week if g else 1

    games = Game.query.filter_by(week=week).order_by(Game.start_time.asc()).all()

    if request.method == 'POST':
        # Save all submitted scores
        edited = 0
        for g in games:
            h_key = f'home_{g.id}'
            a_key = f'away_{g.id}'
            home_val = request.form.get(h_key)
            away_val = request.form.get(a_key)

            # Allow blank (clears score)
            if home_val is None or away_val is None:
                continue

            home_score = home_val.strip()
            away_score = away_val.strip()

            # If both blank -> clear scores
            if home_score == "" and away_score == "":
                if g.final_score_home is not None or g.final_score_away is not None:
                    g.final_score_home = None
                    g.final_score_away = None
                    edited += 1
                continue

            # If both provided, set ints
            try:
                hs = int(home_score)
                as_ = int(away_score)
            except ValueError:
                # ignore bad inputs for this row, but keep processing others
                continue

            if (g.final_score_home != hs) or (g.final_score_away != as_):
                g.final_score_home = hs
                g.final_score_away = as_
                edited += 1

        if edited:
            db.session.commit()
            flash(f"Saved scores for week {week} ({edited} game(s) updated).")
        else:
            flash("No changes detected.")

        return redirect(url_for('main.admin_scores', week=week))

    return render_template('admin_scores.html', week=week, games=games)